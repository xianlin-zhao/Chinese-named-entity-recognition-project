...
def main():
    ...
    parser.add_argument('--model_path', default='./model/hmm.pkl', help='The model will be saved to this path.', type=str)
    ...
    # Load the datasets
    train_word_lists, train_label_lists = preprocess(os.path.join(args.data_path, args.train_file), word2id, tag2id)
    test_word_lists, test_label_lists = preprocess(os.path.join(args.data_path, args.test_file), word2id, tag2id)
    ...
    with open(args.model_path, "wb") as f:
        pickle.dump(model, f)
    pred_label_lists = model.test(test_word_lists, word2id, tag2id)

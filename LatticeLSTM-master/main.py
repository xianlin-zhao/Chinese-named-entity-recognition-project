...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        if seg:
            current_score = f
            print("Dev: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f1: %.4f"%(dev_cost, speed, acc, p, r, f))
        else:
            current_score = acc
        ...
        if current_score > best_dev:
            if seg:
                print("Exceed previous best f1 score:", best_dev)
            else:
                print("Exceed previous best acc score:", best_dev)
            model_name = save_model_dir +'.'+ str(idx + 1) + ".model"
        ...
        if seg:
            print("Test: time: %.2fs, speed: %.2fst/s; acc: %.4f, p: %.4f, r: %.4f, f1: %.4f"%(test_cost, speed, acc, p, r, f))
        else:

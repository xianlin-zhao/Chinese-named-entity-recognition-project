...
def train(args, model, train_features, val_features):
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            loss = model.neg_log_likelihood(input_ids, labels, lengths, input_mask)
            ...
            if num_step % args.eval_steps == 0 and num_step != 0:
                print('Epoch: {:3} | Step: {:6} finish training, start evaluating!'.format(epoch + 1, num_step))
                res = evaluate(args, model, val_features)
                print('Epoch: {:3} | Step: {:6} | Loss: {:8.4f} | P: {:5.2f} | R: {:5.2f} | F1: {:5.2f}'.format(
                    epoch + 1, num_step, loss.item(), res['p'], res['r'], res['f1']))
...
def evaluate(args, model, val_features):
    ...
    with torch.no_grad():
        for batch in val_dataloader:
            ...
            _, pred = model(input_ids, labels, lengths, input_mask)
...
def main():
    ...
    parser.add_argument('--model_path', default='./model/bilstm_crf.pt', help='The model will be saved to this path.', type=str)
    ...
    parser.add_argument('--lr', default=6e-5, help='Learning rate of the optimizer.', type=float)
    ...
    parser.add_argument('--num_epochs', default=32, help='Batch size for training.', type=int)
